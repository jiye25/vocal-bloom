...
{"love":0.0,"longing":0.0,"joy":0.0,"sadness":0.0,"excitement":0.0,"gratitude":0.0}`;
  ...
  try {
    ...
    console.log(`[감정분석] 입력: "${text}" | 응답: ${raw}`);
    ...
    if (total < 0.01) {
      console.log("[감정분석] 감정 없음 → 중립");
      return res.json(NEUTRAL);
    }

    KEYS.forEach(k => { parsed[k] = parsed[k] / total; });
    console.log("[감정분석] 결과:", parsed);
...
app.get("/health", (_, res) => res.json({ status: "ok", model: "gpt-4o-mini" }));

app.listen(PORT, async () => {
  console.log(`\n✅ 감정 분석 서버 실행: http://localhost:${PORT}`);
  console.log(`   API 키: ${apiKey ? `설정됨 (${apiKey.slice(0,8)}...)` : "❌ 없음"}`);
